...
export default function EducationDashboard({ filteredItems, refreshData }) {
  ...
  const handleSave = async (id) => {
    try {
      ...
      console.error(err);
      alert("Failed to update education stock.");
    }
  };

  return (
    <div>
      <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-yellow-500 mb-8">
        <h2 className="text-2xl font-bold text-yellow-600">School & Pupil Resources</h2>
        <p className="text-gray-600 mt-2">Manage the inventory for classrooms, learning materials, and teaching staff here.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredItems.length === 0 ? (
          <p className="text-gray-500 italic col-span-3 text-center">No educational items found.</p>
        ) : (
              ...
              {editingId === item.id ? (
                <div className="mt-4 flex space-x-2">
                  <input 
                    ...
                    className="w-full border-2 border-yellow-400 rounded px-3 py-2 text-gray-800 font-bold focus:outline-none"
                ...
                <button 
                  ...
                  className="w-full mt-4 bg-yellow-50 hover:bg-yellow-100 text-yellow-700 font-bold py-2 px-4 rounded border border-yellow-200 transition"
                >
                  Update Inventory
                </button>
              )}
